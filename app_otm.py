...
if uploaded_file:
    ...
    # Filtra poços do campo selecionado e vazão positiva
    ...
    # Multiselect para restringir poços que não quer fechar
    todos_os_pocos = df_sel['Poço'].tolist()
    protegidos = st.multiselect("🚫 Selecione poços que NÃO devem ser fechados:", todos_os_pocos)

    # Remove os poços protegidos da lista para otimização
    df_otimizacao = df_sel[~df_sel['Poço'].isin(protegidos)]

    if st.button("🚀 Executar Otimização"):
        st.write("⏳ Processando...")
        inicio = time.time()
        resultado = otimizar_fechamento(df_otimizacao, vazao_alvo, dias, max_combo)
        fim = time.time()
